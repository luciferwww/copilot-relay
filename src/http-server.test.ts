...
test('Messages translation dispatches Chat requests and maps the response', async () => {
  ...
  try {
    ...
    assert.equal(upstreamPlan?.endpoint, '/chat/completions');
    const upstreamBody = JSON.parse(Buffer.from(upstreamPlan?.body ?? []).toString('utf8')) as {
...
test('stateless Responses tool continuation survives server replacement', async () => {
  const model: ModelRecord = {
    id: 'gpt-test',
    supported_endpoints: ['/responses'],
    capabilities: { supports: {}, limits: { max_output_tokens: 1024 } },
  };
  const firstRuntime = runtimeFor(model, async () => Response.json({
    id: 'response-id',
    model: 'gpt-test',
    status: 'completed',
    output: [{
  ...
  try {
    ...
    assert.equal(response.status, 200);
    const body = await response.json() as { content: Array<{ type: string; id?: string }> };
    const toolUse = body.content.find((entry) => entry.type === 'tool_use');
    assert.equal(toolUse?.id, 'call-123');
    toolId = toolUse.id;
  } finally {
    await firstServer.close();
  }
  let replayedBody: Record<string, unknown> | undefined;
  const secondRuntime = runtimeFor(model, async (plan) => {
  ...
  try {
    ...
    assert.equal(input[1].call_id, toolId);
    assert.equal(input[2].call_id, toolId);
    assert.equal(input[2].output, 'file contents');
  } finally {
