...
The translator validates required data before the upstream request, but additive protocol changes are not local errors. It maps verified equivalents, passes through unknown structures when Responses can carry them, and otherwise omits optional unknown fields or components with structured warnings that contain names or fixed contexts only. It rejects only when the target request cannot be constructed without inventing required data, when continuation identity or order is inconsistent, or when malformed input or a resource limit makes continuation unsafe.

This compatibility matrix is open to extensions. `top_k`, `stop_sequences`, cache hints, and unknown optional object fields without a Responses equivalent are omitted with warnings. `temperature` and `top_p` are forwarded for upstream validation. Versioned `web_search_*` tools map as one tool family to Responses `web_search`; valid `allowed_domains` moves to `filters.allowed_domains`, while an untranslatable optional filter is omitted with a warning. Unknown tool definitions and tool-choice variants are passed through for upstream validation. An optional custom function, metadata object, output configuration, or image that lacks fields required for its own mapping is omitted rather than failing the request. Model identity, non-empty translated input, continuation association, and bounded request framing remain strict.
...
- **AC12.** Unknown models and requests missing data required to construct a target request return Anthropic 400 errors; malformed or incomplete required model metadata and unavailable metadata without a cache return Anthropic 502 errors. Additive fields, future versioned tools, optional unsupported content, opaque output items, and auxiliary SSE events warn and continue through mapping, passthrough, or omission. No request silently changes the model id or fabricates continuation identity. Tests cover unknown fields without logged values, `web_search_*`, unknown tools, opaque continuation replay, auxiliary SSE events, and strict continuation/resource failures.
