...
### 3.1 Known Continuation Limitation and Future Direction

In v0.2, Messages-to-Responses tool continuation state is held only in the relay process. Restarting the relay discards that state. A Claude Code session whose retained history contains relay-issued `tool_use.id` values may therefore fail its next tool-result turn with an actionable 400 and must currently start a new conversation.
...
A future release must provide finite statelessness at the process boundary: session correctness for an unexpired published continuation must not depend on one Node.js process remaining alive. The intended direction is a versioned, bounded, encrypted, locally recoverable continuation store with atomic publication, the same 24-hour renewable idle TTL, 7-day absolute lifetime, least-recently-used capacity enforcement as the in-memory registry, startup validation and expiry cleanup, and fail-closed handling for corrupted, unknown-version, or undecryptable records. This remains a future requirement; v0.2 does not persist continuation content.
...
The process-local storage rule above is the current v0.2 implementation limit, not the desired final transparency boundary. The future recoverable-store direction and its security constraints are defined in §3.1 and must preserve the same minimal-state and fail-closed properties.
